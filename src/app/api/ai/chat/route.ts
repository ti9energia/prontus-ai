...
function mockReply(text: string, locale: string): string {
  ...
  if (isTime) {
    return L(
      `Hoje o Auronis já devolveu ${minutes} minutos do seu tempo — cerca de ${(minutes / 60).toFixed(1)}h que você não passou digitando.`,
      `Today Auronis already gave you back ${minutes} minutes — about ${(minutes / 60).toFixed(1)}h you didn't spend typing.`,
      `今天 Auronis 已为你节省 ${minutes} 分钟，约 ${(minutes / 60).toFixed(1)} 小时无需手动记录。`,
      `Aujourd'hui, Auronis vous a déjà fait gagner ${minutes} minutes — environ ${(minutes / 60).toFixed(1)} h sans saisie.`,
    );
  }
  if (isAgenda) {
