...
- **O quê:** 5-7 testes de `e2e/landing.spec.ts` (CTA primário, rodapé, páginas legais, FAQ, seletor de idioma) falham de forma consistente e reproduzível em runs de CI limpos (confirmado: não é `MISSING_MESSAGE`, não é `getByLabel('Senha')`, não é contenção — o padrão persiste idêntico entre um run com 30 falhas totais e outro com só 10). Sintomas: "element(s) not found" rápido OU "element is not attached to the DOM" — a página parece sofrer re-renderizações depois do primeiro paint, invalidando referências de nó que o Playwright já tinha capturado.
