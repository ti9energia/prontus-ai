...
- **Correlato (mesmo run):** os scans axe de onboarding/checkout às vezes falham com "Execution context was destroyed … navigation" (corrida do axe com o redirect pós-signup, não a11y real) — segue coberto pelo retry do Playwright. Flakes de timing do checkout sandbox (PIX/boleto) idem.

## 13. Dívida de contraste app-wide + scans axe varrendo DOM incompleto (descoberta 2026-07-03)
- **O quê:** ao tentar estabilizar o flake acima com `await page.waitForLoadState('load')` antes do `axe.analyze()`, o scan passou a varrer a página TOTALMENTE carregada e revelou **dívida de contraste pré-existente e ampla** — 10+ tons de cinza/teal abaixo de AA em **login/checkout/onboarding** (ex.: `#5e6d77`, `#727f8a`, `#778d97`, `#748996` como texto secundário; teals escuros `#13272b`/`#102d30`/`#1d2f32` em texto sobre fundo escuro). Ou seja: os scans axe **passavam por varrer um DOM incompleto** (analyze cedo demais, antes da página assentar), não porque as telas estavam AA.
- **Decisão:** o `waitForLoadState` foi **revertido** — corrigir a11y de contraste app-wide é um esforço dedicado (herda da pendência #9, "text-muted < AA global") e está **fora do escopo** deste hotfix do crash de canvas. Este PR corrige a LANDING (determinística) e deixa o resto rastreado aqui.
- **Como resolver:** PR de remediação de a11y dedicado — (a) tornar o `expectNoSeriousViolations` robusto (esperar a página assentar) p/ o scan ser honesto, e (b) elevar os tokens de texto secundário (`--subtle`/`--muted` e variantes) e os `text-white`-sobre-`bg-brand-600` em todas as telas até AA, com revisão visual. Alto valor, mas precisa de rodada própria.
